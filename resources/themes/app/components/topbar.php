...
$talkNotifications=[];
$talkUnread=0;
$talkTenant=null;$talkTenants=[];
if($currentUser && Access::can('talk.access')){
    try{
        $tenantContext=new TalkTenantContext();$talkTenant=$tenantContext->current((int)$currentUser->id);$talkTenants=$tenantContext->allowed((int)$currentUser->id);
    ...
    }catch(\Throwable){$talkTenant=null;$talkTenants=[];$talkNotifications=[];$talkUnread=0;}
}
$tenantBrand=null;
if($talkTenant&&!empty($talkTenant['brand_color'])&&preg_match('/^#[0-9A-Fa-f]{6}$/',(string)$talkTenant['brand_color'])===1)$tenantBrand=(string)$talkTenant['brand_color'];
?>
...
<?php if($talkTenant): ?><div class="talk-tenant-context"<?= $tenantBrand!==null?' style="--tenant-brand:'.htmlspecialchars($tenantBrand,ENT_QUOTES,'UTF-8').'"':'' ?>><?php if(count($talkTenants)>1 && Access::is('admin')): ?><form method="post" action="/talk/context"><?= $this->csrf() ?><label class="sr-only" for="talk-tenant-select">Administradora</label><select id="talk-tenant-select" name="tenant_id" onchange="this.form.submit()"><?php foreach($talkTenants as $tenant): ?><option value="<?= (int)$tenant['id'] ?>"<?= (int)$tenant['id']===(int)$talkTenant['id']?' selected':'' ?>><?= $this->e((string)$tenant['name']) ?></option><?php endforeach; ?></select></form><?php else: ?><span class="talk-tenant-brand-dot" aria-hidden="true"></span><strong><?= $this->e((string)$talkTenant['name']) ?></strong><?php endif; ?></div><?php endif; ?>
