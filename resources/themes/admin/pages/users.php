...
<section class="studio-page">
    <header class="studio-page-heading"><div><p class="studio-eyebrow">GESTÃO</p><h2>Usuários</h2><p>Gerencie contas, perfis e acesso ao Moves.</p></div><a class="studio-btn primary" href="/studio/users/create">Novo usuário</a></header>
        ...
        <div class="studio-table-wrap"><table>
            ...
            <?php foreach ($users as $user): ?>
                <tr>
                    ...
                    <td><div class="studio-row-actions"><a href="/studio/users/edit/<?= $this->e((string) $user['id']) ?>" aria-label="Editar <?= $this->e($user['name']) ?>">Editar</a><?php if ((int) $user['id'] !== 1): ?><form method="post" action="/studio/users/action"><?= $this->csrf() ?><input type="hidden" name="id" value="<?= $this->e((string) $user['id']) ?>"><input type="hidden" name="action" value="<?= $user['status'] === 'active' ? 'deactivate' : 'activate' ?>"><button type="submit"><?= $user['status'] === 'active' ? 'Desativar' : 'Ativar' ?></button></form><?php endif; ?></div></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table></div>
