...
<header class="studio-topbar studio-topbar-v2">
    ...
    <span class="studio-topbar-spacer"></span>
    <nav class="studio-header-actions" aria-label="Atalhos do painel">
        ...
        <a href="/admin/diagnostics" title="Ajuda e diagnóstico" aria-label="Ajuda e diagnóstico"><?= studio_icon('circle-help') ?></a>
        <a href="/" target="_blank" rel="noopener" title="Visualizar site" aria-label="Visualizar site"><?= studio_icon('globe') ?></a>
        <a class="studio-notification-link" href="/admin/notifications" title="Notificações" aria-label="Abrir notificações<?= $unreadNotifications ? ': '.$unreadNotifications.' não lidas' : '' ?>"><?= studio_icon('bell') ?><b><?= min(99,$unreadNotifications) ?></b></a>
        <?php if ($user !== null): ?><div class="studio-header-profile" role="button" tabindex="0" aria-haspopup="menu" aria-expanded="false"><span class="studio-profile-avatar" aria-hidden="true"><?= $this->e(strtoupper(substr((string) $user->name, 0, 1))) ?></span><span><strong><?= $this->e((string) $user->name) ?></strong></span><?= studio_icon('chevron-down') ?><div class="studio-profile-menu" role="menu"><header><span class="studio-profile-avatar" aria-hidden="true"><?= $this->e(strtoupper(substr((string) $user->name, 0, 1))) ?></span><strong><?= $this->e((string) $user->name) ?></strong><small><?= $this->e((string) $user->email) ?></small></header><nav><a role="menuitem" href="/admin/users"><span>Usuários<small>Contas e permissões</small></span></a><a role="menuitem" href="/admin/settings"><span>Configurações<small>Preferências do MovesOS</small></span></a></nav><footer><form method="post" action="/logout"><?= $this->csrf() ?><button type="submit">Sair</button></form><small>ID #<?= $this->e((string) $user->id) ?></small></footer></div></div><?php endif; ?>
    </nav>
</header>
