...
function jid(value){const digits=String(value||'').replace(/\D/g,'');if(!digits)throw new Error('Destinatário inválido');return digits+'@s.whatsapp.net'}
...
async function start(){
 ...
 sock.ev.on('messages.upsert',async({messages})=>{for(const m of messages||[]){const remote=m?.key?.remoteJid||'';if(!m?.message||m?.key?.fromMe||!remote.endsWith('@s.whatsapp.net'))continue;await postInbound({external_id:m.key.id,from:remote.split('@')[0],push_name:m.pushName||'',type:Object.keys(m.message)[0]||'text',body:textOf(m.message),timestamp:Number(m.messageTimestamp||Math.floor(Date.now()/1000))})}});
}
