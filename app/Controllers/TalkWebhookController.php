    ...
    public function receive(): never
    {
        ...
        try{
            foreach(($payload['entry']??[]) as $entry)foreach(($entry['changes']??[]) as $change){$value=$change['value']??[];$phoneId=trim((string)($value['metadata']['phone_number_id']??''));if($phoneId==='')continue;
                ...
                foreach(($value['messages']??[]) as $message){$type=(string)($message['type']??'text');$media=in_array($type,['image','document','audio','video'],true)&&is_array($message[$type]??null)?$message[$type]:[];$body=$type==='text'?(string)($message['text']['body']??''):(string)($media['caption']??'');$inbound->receiveWhatsApp($channelId,['id'=>(string)($message['id']??''),'from'=>(string)($message['from']??''),'type'=>$type,'body'=>$body,'media_id'=>(string)($media['id']??''),'mime_type'=>(string)($media['mime_type']??''),'filename'=>(string)($media['filename']??'')]);}
                foreach(($value['statuses']??[]) as $status){$externalId=trim((string)($status['id']??''));$delivery=trim((string)($status['status']??''));if($externalId===''||!in_array($delivery,['sent','delivered','read','failed'],true))continue;$m=$pdo->prepare("SELECT id,metadata FROM talk_messages WHERE external_id=:external_id AND direction='outbound' LIMIT 1");$m->execute(['external_id'=>$externalId]);$row=$m->fetch(PDO::FETCH_ASSOC);if(!$row)continue;$metadata=json_decode((string)($row['metadata']??''),true);if(!is_array($metadata))$metadata=[];if((int)($metadata['channel_id']??0)!==$channelId)continue;$current=(string)($metadata['delivery_status']??'');if(!TalkDeliveryStatus::accepts($current,$delivery))continue;$metadata['delivery_status']=$delivery;$metadata['delivery_updated_at']=date('Y-m-d H:i:s');if($delivery==='failed'&&isset($status['errors']))$metadata['delivery_errors']=$status['errors'];elseif($delivery!=='failed')unset($metadata['delivery_errors']);$u=$pdo->prepare("UPDATE talk_messages SET metadata=:metadata WHERE id=:id");$u->execute(['metadata'=>json_encode($metadata,JSON_THROW_ON_ERROR|JSON_UNESCAPED_UNICODE),'id'=>(int)$row['id']]);}
            }
