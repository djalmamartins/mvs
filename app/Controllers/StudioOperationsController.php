...
final class StudioOperationsController extends Controller
{
    public function proposals(): void
    {
        ...
        if(Request::isMethod('POST')){$this->validateCsrf();$id=max(0,(int)Request::post('id',0));$action=(string)Request::post('action','status');$exists=$pdo->prepare('SELECT 1 FROM proposals WHERE id=?');$exists->execute([$id]);if(!$exists->fetchColumn()){Flash::set('error','Proposta não encontrada.');Response::to('/studio/proposals');}$note=mb_substr(trim(strip_tags((string)Request::post('note',''))),0,5000);
            ...
            elseif($action==='note'){if(mb_strlen($note)<2){Flash::set('error','Escreva uma observação.');Response::to('/studio/proposals?view='.$id);}}
            elseif($action==='respond'){if(mb_strlen($note)<10){Flash::set('error','A resposta deve ter ao menos 10 caracteres.');Response::to('/studio/proposals?view='.$id);}$pdo->prepare("UPDATE proposals SET response=?,responded_at=NOW(),status=IF(status='new','contacted',status),assigned_to=? WHERE id=?")->execute([$note,Auth::user()?->id,$id]);}
            ...
            else{Flash::set('error','Ação inválida.');Response::to('/studio/proposals');}
            $pdo->prepare('INSERT INTO proposal_history(proposal_id,action,note,created_by) VALUES(?,?,?,?)')->execute([$id,$action,$note,Auth::user()?->id]);Logger::info('Proposta atualizada no Studio.',['record_id'=>$id,'action'=>$action]);Flash::set('success','Proposta atualizada.');Response::to('/studio/proposals?view='.$id);
        }
    ...
    public function notifications(): void
    {
        ...
        if (Request::isMethod('POST')) {
            ...
            if ($action === 'create') {
                ...
                if (mb_strlen($title) < 3 || mb_strlen($message) < 5) { Flash::set('error', 'Informe um título e uma mensagem válidos.'); Response::to('/studio/notifications'); }
                if ($actionUrl !== '' && !str_starts_with($actionUrl, '/')) { Flash::set('error', 'O link da notificação deve ser interno.'); Response::to('/studio/notifications'); }
                if ($recipientId !== null) {
            ...
            Response::to('/studio/notifications');
        }
