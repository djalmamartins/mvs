...
final class TalkController extends Controller
{
    ...
    private TalkNotificationService $notifications;
    ...
    public function __construct(Router $router)
    {
        ...
        $this->notifications = new TalkNotificationService();
    }
    ...
    public function queue(): void
    {
        ...
        $filters=$this->filters(); $filters['status']='queued';
        $this->page('Fila','queue','Atendimentos aguardando um atendente elegível.',['queue'=>$this->metadata->search($filters),'filters'=>$filters,'queues'=>$this->talk->queues(),'tags'=>$this->metadata->tags()]);
    }
    ...
    public function myTickets(): void{$user=Auth::user();if($user===null){Response::to('/login');}$filters=$this->filters();$this->page('Meus chamados','my-tickets','Atendimentos atualmente sob sua responsabilidade.',['tickets'=>$this->metadata->search($filters,(int)$user->id),'filters'=>$filters,'queues'=>$this->talk->queues(),'tags'=>$this->metadata->tags()]);}
    ...
    public function history(): void{$filters=$this->filters();$filters['status']='closed';$this->page('Histórico','history','Atendimentos finalizados e histórico operacional.',['tickets'=>$this->metadata->search($filters),'filters'=>$filters,'queues'=>$this->talk->queues(),'tags'=>$this->metadata->tags()]);}

    public function notifications(): void
    {
        $user=Auth::user(); if($user===null){Response::to('/login');} $userId=(int)$user->id;
        if(Request::isMethod('POST')){
            $this->requireCsrf('/talk/notifications'); $action=(string)Request::post('action','');
            if($action==='read'){$this->notifications->markRead(max(1,(int)Request::post('notification_id',0)),$userId);}
            elseif($action==='read_all'){$this->notifications->markAllRead($userId);}
            Response::to('/talk/notifications');
        }
        $this->page('Notificações','notifications','Acompanhe transferências, alterações e eventos que exigem sua atenção.',['notifications'=>$this->notifications->unread($userId,100),'notificationCount'=>$this->notifications->unreadCount($userId)]);
    }
    ...
    public function jackSettings(): void{$user=Auth::user();if($user===null){Response::to('/login');}if(Request::isMethod('POST')){$this->requireCsrf('/talk/jack/settings');$this->talk->saveSettings(['jack.enabled'=>Request::post('jack_enabled')!==null?'1':'0','jack.wait_seconds'=>(string)max(0,(int)Request::post('jack_wait_seconds',60)),'jack.transfer_summary'=>Request::post('jack_transfer_summary')!==null?'1':'0'],(int)$user->id);Response::to('/talk/jack/settings?saved=1');}$this->page('Configuração do Jack','jack-settings','Defina quando e como o Jack participa do atendimento.',['settings'=>$this->talk->settings()]);}
    public function queues(): void
    {
        ...
        $queues=$this->talk->queues();$members=[];foreach($queues as $q){$members[(int)$q['id']]=$this->talk->queueMembers((int)$q['id']);}$this->page('Filas e departamentos','queues','Organize departamentos, filas e capacidade de atendimento.',['queues'=>$queues,'departments'=>$this->talk->departments(),'members'=>$members,'users'=>$this->talk->eligibleUsers(),'canManage'=>$manage]);
    }
    public function users(): void{$user=Auth::user();if($user===null){Response::to('/login');}if(Request::isMethod('POST')){$this->requireCsrf('/talk/users');$action=(string)Request::post('action','presence');if($action==='presence'){$this->talk->updatePresence((int)$user->id,(string)Request::post('presence','online'));}elseif($action==='user_settings'){if(!$this->talk->canManage((int)$user->id)){Response::to('/talk/users?error=forbidden');}$this->talk->saveUserSettings(max(1,(int)Request::post('user_id',0)),(string)Request::post('talk_role','agent'),max(1,(int)Request::post('capacity',5)));}Response::to('/talk/users');}$this->page('Usuários e permissões','users','Gerencie atendentes, supervisores e permissões do Talk.',['users'=>$this->talk->usersWithPresence(),'canManage'=>$this->talk->canManage((int)$user->id)]);}
    ...
    public function settings(): void{$user=Auth::user();if($user===null){Response::to('/login');}if(Request::isMethod('POST')){$this->requireCsrf('/talk/settings');$this->talk->saveSettings(['auto_assign.enabled'=>Request::post('auto_assign_enabled')!==null?'1':'0','auto_assign.default_seconds'=>(string)max(5,(int)Request::post('auto_assign_default_seconds',30))],(int)$user->id);Response::to('/talk/settings?saved=1');}$this->page('Configurações','settings','Configurações gerais, canais e regras do Talk.',['settings'=>$this->talk->settings(),'channels'=>$this->talk->channels()]);}

    private function filters(): array{return ['q'=>(string)($_GET['q']??''),'status'=>(string)($_GET['status']??''),'priority'=>(string)($_GET['priority']??''),'queue_id'=>max(0,(int)($_GET['queue_id']??0)),'tag_id'=>max(0,(int)($_GET['tag_id']??0))];}
