...
{
    public function dashboard(): void { $this->page('Visão geral', 'dashboard', 'Acompanhe a operação do atendimento em um único lugar.'); }
    public function queue(): void { $this->page('Fila', 'queue', 'Atendimentos aguardando um atendente elegível.'); }
    public function conversations(): void { $this->page('Conversas', 'conversations', 'Conversas ativas e seus respectivos atendimentos.'); }
    public function contacts(): void { $this->page('Contatos', 'contacts', 'Pessoas identificadas a partir dos canais conectados.'); }
    ...
    private function page(string $title, string $currentPage, string $description): void
    {
        echo $this->view->render('pages/talk-workspace', [
            'title' => $title,
            'productName' => 'Talk',
            'activeProduct' => 'talk',
