...
final class StudioSearchController extends Controller
{
    public function index(): void
    {
        ...
        if(mb_strlen($query)>=2){$pdo=Connection::getInstance();$like='%'.$query.'%';
            ...
            foreach($content->fetchAll(PDO::FETCH_ASSOC) as $row){$module=['page'=>'pages','project'=>'projects','article'=>'articles','highlight'=>'highlights','testimonial'=>'testimonials','faq'=>'faq'][$row['type']]??null;if($module){$results[]=['group'=>'Conteúdo','label'=>$row['label'],'detail'=>$row['type'].' · /'.$row['detail'],'url'=>'/admin/'.$module.'?edit='.$row['id'].'#editor'];}}
