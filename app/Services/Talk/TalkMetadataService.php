...
final class TalkMetadataService
{
    ...
    public function createTag(string $name): int
    {
        ...
        if ($name === '') { throw new \RuntimeException('Informe o nome da tag.'); }
        $slug = $this->slug($name);
        $pdo = Connection::getInstance();
        $existing = $pdo->prepare('SELECT id FROM talk_tags WHERE slug=:slug LIMIT 1');
        $existing->execute(['slug'=>$slug]);
        $id = (int)$existing->fetchColumn();
        if ($id > 0) { return $id; }
        $insert = $pdo->prepare('INSERT INTO talk_tags(name,slug) VALUES(:name,:slug)');
    ...
    {
        Connection::getInstance()->prepare('INSERT IGNORE INTO talk_ticket_tags(ticket_id,tag_id) VALUES(:ticket_id,:tag_id)')->execute(['ticket_id'=>$ticketId,'tag_id'=>$tagId]);
        $this->event($ticketId, $userId, 'ticket.tag_added', ['tag_id'=>$tagId]);
    }

    public function detachTag(int $ticketId, int $tagId, int $userId): void
    {
        Connection::getInstance()->prepare('DELETE FROM talk_ticket_tags WHERE ticket_id=:ticket_id AND tag_id=:tag_id')->execute(['ticket_id'=>$ticketId,'tag_id'=>$tagId]);
    ...
    public function search(array $filters, ?int $assignedUserId = null): array
    {
        $pdo = Connection::getInstance();
        $where = ['1=1']; $params = [];
        $term = mb_substr(trim((string)($filters['q'] ?? '')), 0, 120);
        if ($term !== '') {
            $where[] = '(t.protocol LIKE :term OR t.subject LIKE :term OR c.name LIKE :term OR c.phone LIKE :term)';
            $params['term'] = '%'.$term.'%';
        }
        $status = (string)($filters['status'] ?? '');
        if (in_array($status,['queued','assigned','open','closed'],true)) { $where[]='t.status=:status'; $params['status']=$status; }
        $priority = (string)($filters['priority'] ?? '');
        if (in_array($priority,['low','normal','high','urgent'],true)) { $where[]='t.priority=:priority'; $params['priority']=$priority; }
        $queueId = max(0,(int)($filters['queue_id'] ?? 0));
        if ($queueId > 0) { $where[]='t.queue_id=:queue_id'; $params['queue_id']=$queueId; }
        $tagId = max(0,(int)($filters['tag_id'] ?? 0));
        if ($tagId > 0) { $where[]='EXISTS(SELECT 1 FROM talk_ticket_tags ftt WHERE ftt.ticket_id=t.id AND ftt.tag_id=:tag_id)'; $params['tag_id']=$tagId; }
