...
final class TalkInboundService
{
    ...
    public function receiveWhatsApp(int $channelId,array $message): int
    {
        ...
        try{
            ...
            if($ticketId<=0){$protocol=(new TalkProtocolService())->next($tenantId,null);$s=$pdo->prepare("INSERT INTO talk_tickets(tenant_id,protocol,conversation_id,queue_id,status,priority,subject,source,queued_at) VALUES(:tenant_id,:protocol,:conversation_id,:queue_id,'queued','normal','WhatsApp','whatsapp',NOW())");$s->execute(['tenant_id'=>$tenantId,'protocol'=>$protocol,'conversation_id'=>$conversationId,'queue_id'=>$channel['queue_id']]);$ticketId=(int)$pdo->lastInsertId();}
