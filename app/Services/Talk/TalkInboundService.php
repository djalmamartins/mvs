...
final class TalkInboundService
{
    ...
    public function receiveWhatsApp(int $channelId,array $message): int
    {
        ...
        try{
            ...
            $contact=$pdo->prepare("SELECT id FROM talk_contacts WHERE tenant_id=:tenant_id AND channel='whatsapp' AND external_id=:external_id LIMIT 1");$contact->execute(['tenant_id'=>$tenantId,'external_id'=>$from]);$contactId=(int)$contact->fetchColumn();
            ...
            if($ticketId<=0){$protocol='TALK-'.date('Ymd').'-'.strtoupper(bin2hex(random_bytes(3)));$s=$pdo->prepare("INSERT INTO talk_tickets(tenant_id,protocol,conversation_id,queue_id,status,priority,subject,source,queued_at) VALUES(:tenant_id,:protocol,:conversation_id,:queue_id,'queued','normal','WhatsApp','whatsapp',NOW())");$s->execute(['tenant_id'=>$tenantId,'protocol'=>$protocol,'conversation_id'=>$conversationId,'queue_id'=>$channel['queue_id']]);$ticketId=(int)$pdo->lastInsertId();}
